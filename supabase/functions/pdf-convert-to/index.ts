...
serve(async (req) => {
  ...
  try {
    ...
    console.log("PDF Convert-To function called with:", { 
      filePaths, 
      format
    });
    ...
    const resultUrl = `https://converted-pdf-demo.example/converted_${Date.now()}.${format || 'pdf'}`;
    ...
    console.error("Error in pdf-convert-to function:", error);
